...
\noindent The real paradigm shift occurred in 2012 with the resurgence of Deep Learning, and more specifically of CNNs. Instead of a human defining an edge, with CNNs the computer learned the features on its own from huge datasets Then the evolution branched into two different architectural philosophies: Two-Stage Detectors and One-Stage Detectors. 
...
from simple pattern matching to the sophisticated reasoning capabilities that are now the backbone of the MIKA toolkit. In the last few decades, RNNs and LSTMs have reigned supreme in NLP But these models, able to work on sequences of text, had a fatal flaw. By the time they reached the end of a long sentence, they tended to “forget” the beginning. The vanishing gradient problem meant an AI assistant could nearly never maintain the context needed for complex multi-step instructions, such as finding a medicine bottle and then explaining the dosage. The breakthrough came with the advent of the Transformer architecture in 2017. Transformers came up with "Self-Attention", which removed the requirement for sequential processing. Instead of reading a sentence word by word, the model reads all the words at once and can compute the mathematical relevance of each word to all the others. This taught unheard of context and depth. But that power came at a massive computational cost. We have entered the age of Large Language Models (LLMs), like GPT-4, which are mind-blowingly clever but require huge data centers and constant internet access, making them useless for a privacy-minded, offline assistant like MIKA. 

\noindent This limitation has thus led to the present frontier in NLP. SLMs mark a strategic shift in AI research, emphasizing high density of parameters over size. For instance, the models like Qwen 2.5 (0.5B) that we used for this project reveal that you don’t need billion parameters to have deep reasoning, if you have good quality, diverse datasets. For the MIKA toolkit, the evolution toward SLMsistheenablingfactorforits“Intelligence.” Byintegrating a 0.5-billion parameter model, MIKA can perform on-device intent routing—deciding whether a user’s voice command requires the YOLO segmentation module for currency or the OCR module for medication labels. Unlike its “Large” predecessors, an SLM can be quantized to 4-bit precision and loaded into a mobile device’s RAM without causing thermal throttling or system crashes. This evolution marks the transition of AI from a distant cloud-based service to a localized, personal toolkit that respects user privacy and operates with zero latency in real-world environments..

\section{Evaluation of Related Work \& Research Gaps}
Assistive technology has come a long way, but there remain significant gaps in contextual intelligence, resource efficiency and real-time performance on mobile hardware. This was pioneered by Chypak and Morozov (2023) who generated an audio reading [1] Android based OCR assistant for converting text in the environment to speech Their study was able to demonstrate user-centered capture, but was fundamentally limited by a “Dictionary Gap” and lack of contextual filtering. The system would read all text on a label without discrimination and would not prioritize critical information such as medication dosages [[1]]. Mika does this by leveraging a \textbf{Qwen 2.5 (0.5B)} SLM as a cognitive filter to remove the “textual noise” and only provide relevant summaries through intent-led prompts. Likewise, SeeAround Framework also proposed offline support using heavy architectures such as Inception-V3 and ResNet152 [[2]]. This was too computationally expensive and led to thermal throttling and battery drain on mid-range devices. Mika replaces these CNNs with YOLOv8n (6.1MB) achieving 10x size reduction and better generalization.
\noindent Also considered were the problems of recognition of objects using algorithm SSD MobileNet V2 [[3]]. [[3]] One major limitation identified was the presence of “Localization Errors” in cases of occluded or folded objects, as the system relied on conventional bounding boxes. Mika solves this by switching to YOLOv8-seg (Instance Segmentation) and masking the exact pixel-shape of the currency to make sure it can be recognized reliably. Visually Project (2024) pointed out the issue of “Concurrency Latency”, concurrent model execution crashed devices with 4 GB RAM [[[4]]]. Mika does this with a custom Model Lifecycle Manager and TinyML Wake-Word engine which allows 100Finally, navigation guidance has been attempted in Flutter-based solutions such as Basira application , but with high latency due to inefficient Python/Flask bridges [[5]]. Mika overcomes these limitations by using native C++ memory pointers and optimized TFLite.
